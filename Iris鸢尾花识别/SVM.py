...
data_train, data_test, tag_train, tag_test= train_test_split(iris_dataset['data'],iris_dataset['target'], test_size = 0.3, random_state=1)
...
def draw(clf, x):
    ...
    x3_min, x1_max = x[:, 0].min(), x[:, 2].max()
    
    x4_min, x1_max = x[:, 0].min(), x[:, 3].max()
